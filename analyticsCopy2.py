...
def runAnalytics(in_file):
	...
	wb = openpyxl.load_workbook(in_file) # was in_file[0]
	...
	for row in range(2, sheet.max_row + 1):
		...
		downloads = sheet['O' + str(row)].value #was N
	...
	for row in range(2, sheet.max_row + 1):
		family = sheet['G' + str(row)].value #was F
		downloads = sheet['O' + str(row)].value	#was N
		familyData.setdefault(family, 0)
		familyData[family] += int(downloads)
		
	for row in range(2, sheet.max_row + 1):
		devkit = sheet['M' + str(row)].value#was L
		downloads = sheet['O' + str(row)].value #was N
